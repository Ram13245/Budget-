...
Budget::Budget(float amt, const time_t& sDate, const time_t& eDate,
		vector<Category>& cats) // : totalAmount(0), startDate(0), endDate(0)
...
void Budget::changeCategories(vector<Category> cats) {	//needs testing
	...
	for (i = 0; i < original_cat_names.size(); i++) {
		if (!(find(new_cat_names.begin(), new_cat_names.end(),
				original_cat_names[i]) != new_cat_names.end())) {
...
void Budget::deleteExpenses(vector<int> ids) {//again, I think that category needs to be specified, though the ids should be unique
	...
	for (map<string, vector<Expense>>::iterator it = expenses->begin();
			it != expenses->end(); it++) {
		//iterate through expenses vector in each
		i = 0;
		while (i < it->second.size()) {
			if (find(ids.begin(), ids.end(), it->second[i].getID())
					!= ids.end()) {
...
void Budget::addExpenses(const vector<Expense>& exp) {
	...
	for (Expense e : exp) {
		...
		if (expenses->count(cat) == 1) {
			...
			expenses->insert(
					map<string, vector<Expense>>::value_type(cat,
							new_category));
		}
	}
}
